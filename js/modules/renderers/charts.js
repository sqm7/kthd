...
export function renderSalesVelocityChart(analysisResult, mainData) {
    ...
    const getRoomCategory = (record) => {
        ...
        if (buildingType.includes('廠辦')) return '廠辦'; // 確保廠辦判斷存在
    ...
    // ▼▼▼ 【重要更新】強化房型篩選器的渲染與互動邏輯 ▼▼▼
    const renderRoomTypeFilters = () => {
        // 使用 allRoomTypes (從後端來的權威資料源) 來建立篩選器
        dom.roomTypeFilterContainer.innerHTML = allRoomTypes.map(type => {
            ...
            // 預設勾選狀態：除了'其他'之外，都預設勾選
            const isChecked = type !== '其他'; 
            return `
                <label class="inline-flex items-center mr-4 mb-2 cursor-pointer">
                    ...
                    <span class="ml-2 text-sm" style="color: ${CHART_COLORS[colorIndex]}; font-weight: 500;">${type}</span>
                </label>
            `;
        }).join('');
    ...
    // 初始化時，根據篩選器的初始狀態設定圖表可見性
    const setInitialVisibility = () => {
        if (!currentChart) return;
        const checkboxes = dom.roomTypeFilterContainer.querySelectorAll('input[type="checkbox"]');
        checkboxes.forEach(checkbox => {
            const type = checkbox.dataset.roomType;
    ...
    // 執行渲染
    renderRoomTypeFilters(); // 先渲染篩選器
    createChart(currentView); // 再根據篩選器狀態渲染圖表
    if(currentChart) setInitialVisibility();
        ...
        data: {
            ...
            datasets: roomTypes.map((type, i) => ({
                ...
                backgroundColor: (context) => {
                    if (!context.raw || context.raw.v === 0) return 'rgba(240, 240, 240, 0.5)';
                    const alpha = Math.min(1, 0.15 + context.raw.v / 10);
